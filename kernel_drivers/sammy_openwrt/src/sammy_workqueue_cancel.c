...
static irqreturn_t irq_handler(int irq, void *dev){
	...
	schedule_delayed_work(&mywork, msecs_to_jiffies(10000));
...
static int __init sammy_gpio_workqueue_init(void){
	...
	// Use the INIT_WORK 
	INIT_DELAYED_WORK(&mywork, custom_task);
...
static void __exit sammy_gpio_workqueue_exit(void){
	...
	// cleanup any pending Workqueue
	if(delayed_work_pending(&mywork)){
		printk(KERN_INFO "SAMMY: Workqueue pending, cancelling it\n");
