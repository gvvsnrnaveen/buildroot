...
static void sammy_tasklet_isr(struct tasklet_struct *);
static void free_gpio_pins(void);

DECLARE_TASKLET(sammy_tasklet, sammy_tasklet_isr);

// Bottom half Tasklet ISR
static void sammy_tasklet_isr(struct tasklet_struct *ptr) {
	int value = 0;
	int i = 0;
...
static int __init sammy_gpio_tasklet_init(void) {
	...
	printk(KERN_INFO "SAMMY: sammy gpio tasklet sample init success\n");
...
static void __exit sammy_gpio_tasklet_exit(void) {
	...
	// Free GPIO pins
	free_gpio_pins();
	printk(KERN_INFO "SAMMY: sammy gpio tasklet module exit success\n");
}
